...
# Turn off that any plot might be shown if not desired
plt.ioff()

class StockObject():
    '''
    Class to acquire stock data, collect rates and plot a corresponding chart
    '''
    def __init__(self, stock, cur_date, exchange_rate, user_currency, chart_dir, show_plot=False,
         ...
         # Interpolate missing data (holidays/weekends) to match with currency exchange rate data
         ...
         # Make sure correct buy price is set
         self.stock_data['Close'][self.buy_date] = self.buy_price

         # If so, correct for exchange rate
         ...
         # Compute sliding average(s) for n day mean line
    ...
    def plot_stock(self):
        ...
        self.f.suptitle(self.name + ': '
            ...
            + 'buy price: ' + str(round(self.buy_price,2)) + self.currency + '(' + str(round(self.anual_interest,2)) + ' % p.a.)'
        ...
        plt.savefig(os.path.join(self.chart_dir,self.name + '.pdf'))
        if self.show_plot:
            plt.show()
...
class WealthObject():
    ...
    '''
    def __init__(self, data, buy_dates, chart_dir, show_plot=False, report_days=[30, 200, 365], show_interval=0.05):
        ...
        plt.savefig(os.path.join(self.chart_dir, 'WEALTH.pdf'))
        if show_plot:
            plt.show()
...
if __name__ == '__main__':
    ...
    # If not present, create dir for stock stock charts
    ...
    for stock in stocks:
        ...
        portfolio[stock] = StockObject(stocks[stock], cur_date, exchange_rate,
                                       user_currency, chart_dir, show_charts, report_days,
                                       window_size)
    ...
    wealth = WealthObject(hist_data, buy_dates, chart_dir, show_wealth_plot, report_days)
